...
function spawner.canKill(kill)
    canKill = kill
end
function spawner.speed(ss)
    speed = ss
end
function spawner.pauseTime(t)
    pauseTime = t
end
...
--Controller
while wait(0.001) and spawned do
--rs.Heartbeat:Connect(function()
	...
	cPart.CFrame = cPart.CFrame + cPart.CFrame.LookVector * speed
    local oldTN = targetNode
    if getDistance(cPart.CFrame.Position, addY(targetNode.CFrame.Position, 4)) <= speed then
